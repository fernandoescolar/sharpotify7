...
namespace Sharpotify.Protocol.Channel
{
    public class Channel
    {
        ...
        #endregion
        #region Factory
        public Channel(string name, ChannelType type, IChannelHandler handler)
        {
        ...
        public static void Process(byte[] payload)
        {
            ...
            if (channel.State == ChannelState.STATE_HEADER)
            {
                ...
                while (consumedLength < length)
                {
                    ...
                    if (channel._handler != null)
                    {
                        byte[] buffer = new byte[headerLength];
                        Array.Copy(payload, offset, buffer, 0, headerLength);

                        channel._handler.ChannelHeader(channel, buffer);
                    }
            ...
            if (length == 0)
            {
                ...
                if (channel._handler != null)
                {
                    channel._handler.ChannelEnd(channel);
                }
            }
            else
            {
                if (channel._handler != null)
                {
                    ...
                    channel._handler.ChannelData(channel, buffer);
                }
            }
        ...
        public static void Error(byte[] payload)
        {
            ...
            if (channel._handler != null)
            {
                channel._handler.ChannelError(channel);
            }
